    ...
    try {
        ...
        switch (method) {
            case "GET":
                ...
                let customerBikes;
                try {
                    customerBikes = await db
                        .collection("bikes")
                        .find(
                            { store: storeId, customerIdentifier: Number(customerIdentifier) },
                            { projection: { store: false } }
                        )
                        .toArray()
                } catch (error) {
                    console.log(error)
                ...
                customerBikes = customerBikes.map(bike => {
                ...
                if (customerBikes.length === 0) {
                    const error: ApiErrorResponse = {
                        statusCode: 404,
                        message: `No bikes belonging to customer were found.`,
                    }
                    res.status(404).json(error)
                    return
                }
                res.status(200).json(customerBikes);
                break
            case "POST":
                const { customerBike } = body;

                //update customer with request data
                const { insertedId } = await db
                    .collection("bikes")
                    .insertOne(
                        { store: storeId, ...customerBike },
                    );

                res.status(200).json({ ...customerBike, id: insertedId.toString() });
                break
            case "DELETE":
